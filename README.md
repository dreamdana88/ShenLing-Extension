...
情感结果会先按当前 swipe 暂存；面板会显示当前选中页的待确认变化。继续下一轮生成前，插件只把当前选中 swipe 的结果写入正式档案。

`changed=false` 不显示待确认卡；如果该页最终被选中，会清理同一楼层旧记录，不新增档案。

情感档案开启时，插件会把每个角色最新一条情感档案注入给主 API。历史变化仍保存在面板中，方便查看角色关系演变。

关闭“情感档案”时，小总结请求中不会发送情感判断相关提示词。
