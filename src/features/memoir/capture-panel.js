...
import { runCaptureGeneration } from './capture-workflow.js';
import {
...
export function configureCapturePanel(options = {}) {
...
function renderCaptureForm(capture) {
  ...
  return `
    <div class="slx-capture-form">
      ...
      <button
        class="slx-primary-btn slx-capture-generate-btn ${generating ? 'is-loading' : ''}"
        type="button"
...
function renderCaptureRegion(capture) {
  return `
    <div class="slx-capture-region" data-slx-capture-region>
      ...
      <details class="slx-capture-advanced" ${uiState.advancedOpen ? 'open' : ''} data-slx-capture-advanced>
        <summary>高级诊断（开发期）</summary>
        <div class="slx-capture-advanced-body">
          ${renderCaptureStageTestPanel()}
        </div>
      </details>
    </div>
...
export function bindCaptureEvents(panelRoot) {
  ...
  if (region) {
    ...
    region.querySelector('[data-slx-capture-advanced]')?.addEventListener('toggle', event => {
      uiState.advancedOpen = event.currentTarget.open;
    });
  }
